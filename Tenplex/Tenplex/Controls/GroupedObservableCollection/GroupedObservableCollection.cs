...
namespace Tenplex.Controls
{
        ...
        where TKey : IComparable<TKey>
    {
        ...
        public Func<TElement, object> Order;

        /// <summary>
        /// This is used as an optimization for when items are likely to be added in key order and there is a good probability
        ...
        public GroupedObservableCollection(Func<TElement, TKey> readKey, IEnumerable<TElement> items, Func<TElement, object> order)
            : this(readKey)
        {
            Order = order;
            foreach (var item in items)
        ...
        public IEnumerable<TElement> AllItems => this.SelectMany(g => g);

        public int ItemCount => this.SelectMany(g => g).Count();
        ...
        public void AddItem(TElement item)
        {
            ...
            var sortedList = group.ToList();
            sortedList.Add(item);

            if (Order != null)
                sortedList = sortedList.OrderBy(a => Order(a)).ToList();

            var i = sortedList.IndexOf(item);
            group.Insert(i, item);
        }
